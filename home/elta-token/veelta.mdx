...
When users lock their ELTA, they receive veELTA. Users' veELTA value is proportional to the amount and duration of veELTA staked (up to a maximum of 2 years). Each wallet address is associated with a single veELTA expiry date.

veELTA uses a linear decay model — meaning its value will decrease over time — and reaches zero once the lock duration is over. Once this happens, the full initial ELTA deposit will then be unlocked.
